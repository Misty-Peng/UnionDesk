...
> **与 PRD 的关系**：`产品需求文档 v1.0.md` §5 描述目标域数据模型（`identity_subject` / `staff_account` 等）；本文档描述**当前仓库 Flyway 已落库**的表结构与命名（含过渡期 `user_account` 等）。二者差异与演进路径以 `数据模型迁移策略.md` 为准。
...
- `role`：角色定义表，当前角色包含 `customer`、`agent`、`domain_admin`、`super_admin`、`platform_admin`、`security_auditor`；目标态中 `platform_admin` 为平台级最高权限，`super_admin` 作为业务域最高角色使用，迁移路径以 `数据模型迁移策略.md` 为准。
- `user_global_role`：全局角色绑定表。
- `user_domain_role`：业务域角色绑定表。
- `customer_business_domain_access`：客户业务域可见/申请关系表。

当前项目里，`user_account` 还承担离职池信息的归档入口，所以离职字段必须在表结构里保留。当前这一轮只先落平台组织主数据与负责人信息，用户归属组织关系后续再补。
...
4. `auth_login_log`：`sid, created_at`、`user_id, created_at`、`event_type, created_at`
...
- 旧版 DDL 文件已删除，当前 schema 最终态由 Flyway 迁移脚本维护；新旧模型差异见 `数据模型迁移策略.md`。
